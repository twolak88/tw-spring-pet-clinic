...
public class OwnerControllerTest {
	...
	void testFindOwners() throws Exception {
		this.mockMvc.perform(get("/owners/find"))
			...
			.andExpect(view().name(VIEWS_FIND_OWNERS))
	...
	void testProcessFindFormReturnOne() throws Exception {
		when(this.ownerService.findAllByLastNameLike(anyString())).thenReturn(Stream.of(Owner.builder().id(OWNER_ID).build()).collect(Collectors.toSet()));
	...
	void testShowOwner() throws Exception {
		...
		this.mockMvc.perform(get("/owners/" + OWNER_ID))
			.andExpect(status().isOk())
